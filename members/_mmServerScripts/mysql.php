...
*/
	...
	function Open()
	{
	  $this->connectionId = mysql_connect($this->hostname, $this->username, $this->password);
		...
		{
			$this->isOpen = ($this->database == "") ? true : mysqli_query($connection, $query)($this->database, $this->connectionId);
		}
		else
		{
			$this->isOpen = false;
	...
	function GetTables($table_name = '')
	{
		...
		if ($this->isOpen && isset($this->connectionId) && is_resource($this->connectionId)){
			...
			$results = mysqli_query($connection,$sql, $this->connectionId) or $this->HandleException();
	...
	function GetColumnsOfTable($TableName)
	{
		...
		$result = mysqli_query($connection,$query) or $this->HandleException();
	...
	function ExecuteSQL($aStatement, $MaxRows)
	{
		...
		$result = mysqli_query($connection,$aStatement) or $this->HandleException();
