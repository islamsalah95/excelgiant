...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE requestes SET requeste_type=%s, details=%s, viewed=%s, create_date=%s, edit_date=%s, create_by=%s, edit_by=%s WHERE id=%s",
...
mysqli_query($connection, $query)($database_connection, $connection);
$query_Recordset2 = sprintf("SELECT * FROM requestes WHERE id = %s", GetSQLValueString($colname_Recordset2, "int"));
$Recordset2 = mysqli_query($connection,$query_Recordset2) or die(mysqli_error($connection));
$row_Recordset2 = mysqli_fetch_assoc($Recordset2);
$totalRows_Recordset2 = mysqli_num_rows($Recordset2);

mysqli_query($connection, $query)($database_connection, $connection);
$query_RecordsetResquestes = "SELECT * FROM payments_type";
$RecordsetResquestes = mysqli_query($connection,$query_RecordsetResquestes) or die(mysqli_error($connection));
$row_RecordsetResquestes = mysqli_fetch_assoc($RecordsetResquestes);
$totalRows_RecordsetResquestes = mysqli_num_rows($RecordsetResquestes);
?>
<?php
      ...
      <div class="w3-container w3-card w3-white w3-margin-bottom">
        ...
        <div class="w3-container"><br>
          ...
          <table width="100%" border="0" cellspacing="4" cellpadding="4">
            <tr>
              <td align="left">&nbsp;
                <form method="post" name="form1" action="<?php echo $editFormAction; ?>create_by=<?php echo $row_Recordset1['id']; ?>">
                  <table align="center">
                    <tr valign="baseline">
                      <td nowrap align="right" valign="top">نوع الطلب&quot;</td>
                      <td><label for="requeste_type"></label>
                        <label for="requeste_type"></label>
                        <select name="requeste_type" id="requeste_type">
                          <?php
do {  
?>
                          <option value="<?php echo $row_RecordsetResquestes['name']?>"<?php if (!(strcmp($row_RecordsetResquestes['name'], $row_Recordset2['requeste_type']))) {echo "selected=\"selected\"";} ?>><?php echo $row_RecordsetResquestes['name']?></option>
                          <?php
} while ($row_RecordsetResquestes = mysqli_fetch_assoc($RecordsetResquestes));
  $rows = mysqli_num_rows($RecordsetResquestes);
  if($rows > 0) {
      mysql_data_seek($RecordsetResquestes, 0);
	  $row_RecordsetResquestes = mysqli_fetch_assoc($RecordsetResquestes);
  }
?>
                        </select></td>
                    ...
                    <tr valign="baseline">
                      <td align="right" valign="top" nowrap>تفاصيل الطلب:</td>
                      <td><textarea name="details" cols="32" rows="10"><?php echo htmlentities($row_Recordset2['details'], ENT_COMPAT, 'UTF-8'); ?></textarea></td>
                    </tr>
                    <tr valign="baseline">
                      <td nowrap align="right"><input type="hidden" name="edit_date" value="<?php echo date("Y-m-d h:i:s"); ?>" size="32">
                        <input type="hidden" name="create_by" value="<?php echo htmlentities($row_Recordset2['create_by'], ENT_COMPAT, 'UTF-8'); ?>" size="32">
                      <input type="hidden" name="create_date" value="<?php echo htmlentities($row_Recordset2['create_date'], ENT_COMPAT, 'UTF-8'); ?>" size="32">                        <input type="hidden" name="edit_by" value="<?php echo $row_Recordset1['id']; ?>" size="32">                      <input type="hidden" name="viewed" value="<?php echo htmlentities($row_Recordset2['viewed'], ENT_COMPAT, 'UTF-8'); ?>" size="32"></td>
...
mysqli_free_result($RecordsetResquestes);
?>
