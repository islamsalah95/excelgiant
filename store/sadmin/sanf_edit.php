...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE products SET category=%s, subCategory=%s, productName=%s, productCompany=%s, productPrice=%s, productPriceBeforeDiscount=%s, productDescription=%s, image=%s, photo=%s, picture=%s, shippingCharge=%s, productAvailability=%s, postingDate=%s, updationDate=%s WHERE id=%s",
...
mysqli_select_db($connection, "excelgia_mahmdata");
$query_Recordset2 = sprintf("SELECT * FROM products WHERE id = %s", GetSQLValueString($colname_Recordset2, "int"));
...
do {  
?>
              <option value="<?php echo $row_Recordset1['categoryName']?>" <?php if (!(strcmp($row_Recordset1['categoryName'], htmlentities($row_Recordset2['category'], ENT_COMPAT, 'utf-8')))) {echo "SELECTED";} ?>><?php echo $row_Recordset1['categoryName']?></option>
              <?php
