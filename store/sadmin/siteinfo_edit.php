...
$query_Recordset1 = sprintf("SELECT * FROM siteinfo WHERE id = %s", GetSQLValueString($colname_Recordset1, "int"));
...
mysqli_select_db($connection, "excelgia_mahmdata");
$query_Recordset2 = sprintf("SELECT * FROM siteinfo WHERE sub_syndicate = %s", GetSQLValueString($colname_Recordset2, "text"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE siteinfo SET first_name=%s, bname=%s, cname=%s, last_name=%s, mail=%s, username=%s, password=md5(%s), address=%s, country=%s, salary=%s, phone=%s, mobile=%s, details=%s, grad_img=%s, per_img=%s, sign_img=%s, grad_date=%s, grad_place=%s, id_card=%s, work_place=%s, create_date=%s, edit_date=%s, last_login=%s, create_by=%s, edit_by=%s, group_id=%s, active=%s, sub_syndicate=%s, job=%s, onus=%s WHERE id=%s",
                       ...
                       GetSQLValueString($_POST['grad_date'], "text"),
                       ...
                       GetSQLValueString($_POST['edit_date'], "text"),
                       GetSQLValueString($_POST['last_login'], "text"),
  ...
  $updateGoTo = "siteinfo_view.php?sub_syndicate=" . $row_Recordset1['sub_syndicate'] . "";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<form action="<?php echo $editFormAction; ?>tttttt" method="post" enctype = "multipart/form-data" name="form1" id="form1">
    ...
    function updateFileName2() {
        ...
        grad_img1.value = grad_img.value.substring(fileNameIndex + 1);
          ...
          <tr valign="baseline">
            <td nowrap="nowrap" align="right">عبارة الشعار:</td>
            <td><textarea name="bname" cols="32" rows="5" id="bname"><?php echo htmlentities($row_Recordset1['bname']); ?></textarea></td>
          </tr>
          <tr valign="baseline">
            <td nowrap="nowrap" align="right">العنوان:</td>
            <td><input type="text" name="work_place" value="<?php echo htmlentities($row_Recordset1['work_place'], ENT_COMPAT, 'utf-8'); ?>" /></td>
          </tr>
          <tr valign="baseline">
            <td nowrap="nowrap" align="right">رقم الموبايل:</td>
            <td><input type="text" name="country" value="<?php echo htmlentities($row_Recordset1['country'], ENT_COMPAT, 'utf-8'); ?>" /></td>
          </tr>
          <tr valign="baseline">
            <td nowrap="nowrap" align="right">البريد الإلكتروني:</td>
            <td><input type="text" name="grad_place" value="<?php echo htmlentities($row_Recordset1['grad_place'], ENT_COMPAT, 'utf-8'); ?>" /></td>
          </tr>
          <tr valign="baseline">
            <td nowrap="nowrap" align="right">الفيسبوك:</td>
            <td><input type="text" name="create_by" value="<?php echo htmlentities($row_Recordset1['create_by'], ENT_COMPAT, 'utf-8'); ?>" /></td>
          </tr>
          <tr valign="baseline">
            <td nowrap="nowrap" align="right">لينكد إن:</td>
            <td><input name="edit_by" type="text" id="create_by" value="<?php echo $row_Recordset1['edit_by']; ?>" /></td>
          </tr>
          <tr valign="baseline">
            <td nowrap="nowrap" align="right">وصف مختصر:</td>
            <td><textarea name="edit_date" id="editor" cols="32" rows="4"><?php echo $row_Recordset1['edit_date']; ?></textarea>
            <script>
        ClassicEditor
