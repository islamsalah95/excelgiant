...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE agents SET name=%s, email=%s, contactno=%s, password=%s, shippingAddress=%s, shippingState=%s, shippingCity=%s, shippingPincode=%s, billingAddress=%s, billingState=%s, billingCity=%s, billingPincode=%s, regDate=%s, updationDate=%s WHERE id=%s",
...
mysqli_select_db($connection, "excelgia_mahmdata");
$query_Recordset1 = sprintf("SELECT * FROM agents WHERE id = %s", GetSQLValueString($colname_Recordset1, "int"));
